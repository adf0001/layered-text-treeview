...
return the first added, or `null` if the layered-text is empty;
*/
var addNode = function (el, content, options, childrenContainer) {
	...
	if ((typeof text !== "string") ||
		...
		throw Error("invalid data, " + text + ", " + property);
	}
...
return the first added, or `null` if the layered-text is empty;
	...
	childrenContainer
		internal using, refer to treeview-model.addNode();
		
		when childrenContainer is true, the parent should be expanded by caller;
*/
var addLayeredText = function (el, layeredText, options, childrenContainer) {
	//layeredText
	if (!(layeredText instanceof Array)) throw Error("not a layered-text");
	...
	if (elFirst && !options?.insert && !childrenContainer && getToExpandState(el) !== false) {
		setToExpandState(elFirst.parentNode, false);
	}

	return elFirst || null;		//return the first
}
